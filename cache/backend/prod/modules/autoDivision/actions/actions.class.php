...
require_once(dirname(__FILE__).'/../lib/BaseDivisionGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseDivisionGeneratorHelper.class.php');
 ...
 */
class autoDivisionActions extends sfActions
{
  public function preExecute()
  {
    $this->configuration = new divisionGeneratorConfiguration();
    ...
    if ($condition = $this->configuration->getCondition($this->getActionName()))
    {
      ...
      else
      {
        ...
        $this->forward404Unless($this->division->$condition());
      }
    }

    $this->dispatcher->notify(new sfEvent($this, 'admin.pre_execute', array('configuration' => $this->configuration)));

    $this->helper = new divisionGeneratorHelper();
  }
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    $this->division = $this->form->getObject();
  }
  ...
  {
    $this->division = $this->getRoute()->getObject();
    $this->form = $this->configuration->getForm($this->division);
  }
  ...
  {
    return $this->getUser()->getAttribute('division.filters', $this->configuration->getFilterDefaults(), 'admin_module');
  }

  protected function setFilters(array $filters, $filtering = false)
  {
    $this->getUser()->setAttribute('division.filtering', $filtering, 'admin_module');
    
    return $this->getUser()->setAttribute('division.filters', $filters, 'admin_module');
  }

  protected function getPager()
  {
    $pager = $this->configuration->getPager('Division');
  ...
  {
    $this->getUser()->setAttribute('division.page', $page, 'admin_module');
  }

  protected function getPage()
  {
    return $this->getUser()->getAttribute('division.page', 1, 'admin_module');
  }
  ...
  protected function addSortCriteria($criteria)
  {
    ...
    $column = $this->configuration->getSortColumnNameForField(strtolower($sort[0]),'Division');
    if ('asc' == $sort[1])
    {
  ...
  {
    if (!is_null($sort = $this->getUser()->getAttribute('division.sort', null, 'admin_module')))
    {
      return $sort;
    }
    ...
    return $this->getUser()->getAttribute('division.sort', null, 'admin_module');
  }
  ...
  protected function setSort(array $sort)
  {
    ...
    $this->getUser()->setAttribute('division.sort', $sort, 'admin_module');
  }
  ...
  public function executeDoExportationPages(sfWebRequest $request)
  {
    ...
    return $this->renderPartial('division/exportation_pages', array('pager' => $this->exportationPager, 'exportUrl' => 'doExportation'));
  }
  ...
  public function executeCreateUserExportation(sfWebRequest $request)
  {
    ...
    if ($this->form->isValid())
    {
      ...
      $this->getUser()->setAttribute('division.exportation_form_values', $values);
  ...
  {
    $this->pageNumber            = $request->getParameter('page');
    $this->exportationFormValues = $this->getUser()->getAttribute('division.exportation_form_values', array());
  ...
  {
    $pager = $this->configuration->getExportationPager('Division', $type);
  ...
  {
    $this->getUser()->setAttribute('division.exportation_page', $page, 'admin_module');
  }

  protected function getExportationPage()
  {
    return $this->getUser()->getAttribute('division.exportation_page', 1, 'admin_module');
  }
